...
namespace LibChromeDotNet.HTML5
{
    ...
    public class WebCRAppHost : IWebAppHost
    {
        public static IWebAppHost Create(IWebApp app) => new WebCRAppHost(ChromeLauncher.CreateForPlatform(), app);

        private WebCRAppHost(IChromeLauncher launcher, IWebApp app)
        {
            _App = app;
            _Launcher = launcher;
        }
        ...
        {
            public WebAppContext(WebCRAppHost host)
            {
                _Host = host;
            }

            private WebCRAppHost _Host;
